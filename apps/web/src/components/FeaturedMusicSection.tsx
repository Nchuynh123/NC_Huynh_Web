      ...
      <Container maxWidth="lg">
        ...
        >
          {(['spotify', 'youtube'] as Platform[]).map((p) => {
            ...
            const label = p === 'spotify' ? 'Spotify' : 'YouTube';
          ...
          {isLoading
            ? [0, 1].map((i) => (
              <Grid key={i} item xs={12} sm={6}>
                <Skeleton variant="rounded" height={280} sx={{ borderRadius: 6, bgcolor: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)' }} />
              </Grid>
            ))
            : featured.map((album) => (
              <Grid key={album.id} item xs={12} sm={6}>
